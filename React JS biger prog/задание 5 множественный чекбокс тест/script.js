...
constructor() {
        ...
        doneTest () {
            ...
            for (let i =0 ; i<this.state.questions.length; i++) {
					...
					alert (this.state.inputsVal[i].equals(this.state.rigthA[i]))
					
                if (this.state.inputsVal[i].equals(this.state.rigthA[i]) ) {
						
						const rightAnswerV = this.state.questions[i].rightAnswer.map ((elem,index)=>{
								return (<p style={{color: 'green'}}>ваш ответ {this.state.questions[i].answers[elem]}  И это правильно</p>)
								
							})
						
                        this.state.questions[i].button = rightAnswerV
                        this.setState ({questions:this.state.questions})


                    } else {
						
						const wrongAnswerV = this.state.questions[i].rightAnswer.map ((elem,index)=>{
								return (<p style={{color: 'red'}}>ваш ответ 1111111 а правильный {this.state.questions[i].answers[elem]}</p>)
